...
while running:
    for event in pygame.event.get():
        ...
        if current_turn == "player" and event.type == pygame.KEYDOWN:
            if event.key == pygame.K_UP:
                ...
                enemy_hp = player_attack_enemy(player_pos, enemy_pos, enemy_hp, player_attack, damage_indicators, CELL_SIZE)
    ...
    if current_turn == "enemy" and enemy_hp > 0:
        if abs(enemy_pos[0] - player_pos[0]) + abs(enemy_pos[1] - player_pos[1]) == 1:
            player_hp = enemy_attack_player(player_pos, enemy_pos, player_hp, enemy_attack, damage_indicators, CELL_SIZE)
            if check_game_over(player_hp, enemy_hp):
                running = False
            else:
                current_turn = "player"
